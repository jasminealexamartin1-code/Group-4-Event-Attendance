...
if ($action === 'register_attendee') {
    ...
    try {
        ...
        $newAttendeeId = (int)db()->lastInsertId();
    } catch (PDOException $e) {
        json_out(['ok' => false, 'error' => 'Database Error: ' . $e->getMessage()], 500);
    }
    ...
    try {
        ...
        $eventData = ['name' => 'Event', 'date' => 'TBA', 'location' => 'TBA'];
    }
    ...
    $mailSent = sendConfirmationEmail($attendeeData, $eventData);

    json_out(['ok' => true, 'mail_sent' => $mailSent]);
}
